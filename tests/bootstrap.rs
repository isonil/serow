...
#[test]
fn docs_check_keeps_long_markdown_fences_open_across_shorter_runs() {
    let dir = unique_temp_dir("serow-docs-long-code-fences");
    fs::create_dir_all(dir.join("docs")).expect("create docs dir");
    fs::create_dir_all(dir.join("Progress")).expect("create progress dir");
    fs::write(
        dir.join("README.md"),
        concat!(
            ...
            "[not a link](missing-fenced.md)\n",
            "```\n",
            "[still not a link](missing-after-shorter-fence.md)\n",
            "````\n"
        ),
    )
    .expect("write readme");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_serow"))
        ...
        .args(["docs", "--check", "--json"])
        .output()
        .expect("run serow docs --check with long markdown code fence");
    ...
    assert!(!stdout.contains("missing-fenced.md"), "{stdout}");
    assert!(
        !stdout.contains("missing-after-shorter-fence.md"),
        "{stdout}"
    );
...
#[test]
fn docs_check_matches_unicode_reference_labels_case_insensitively() {
    let dir = unique_temp_dir("serow-docs-unicode-reference-labels");
    fs::create_dir_all(dir.join("docs")).expect("create docs dir");
    fs::create_dir_all(dir.join("Progress")).expect("create progress dir");
    fs::write(
        dir.join("README.md"),
        concat!(
            "# Fixture\n\n",
            "Unicode reference: [Language][CAFÉ].\n\n",
            "[Café]: docs/language.md#language\n"
        ),
    )
    .expect("write readme");
    fs::write(dir.join("AGENTS.md"), "# Agents\n").expect("write agents");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_serow"))
        ...
        .args(["docs", "--check", "--json"])
        .output()
        .expect("run serow docs --check with unicode reference labels");
    ...
    assert!(stdout.contains("\"markdown_links_ok\": true"), "{stdout}");
    assert!(stdout.contains("\"broken_links\": []"), "{stdout}");
    assert!(!stdout.contains("missing reference definition"), "{stdout}");
...
#[test]
fn docs_check_handles_angle_bracketed_unbalanced_parentheses() {
    let dir = unique_temp_dir("serow-docs-angle-parentheses-links");
    fs::create_dir_all(dir.join("docs")).expect("create docs dir");
    fs::create_dir_all(dir.join("Progress")).expect("create progress dir");
    fs::write(
        dir.join("README.md"),
        concat!(
            "# Fixture\n\n",
            "Valid: [Topic](<docs/topic(left.md#topic-left> \"Topic (Left)\").\n",
            "Broken anchor: [Missing](<docs/topic(left.md#missing-section> \"Missing\").\n"
        ),
    )
    .expect("write readme");
    fs::write(dir.join("AGENTS.md"), "# Agents\n").expect("write agents");
    ...
    fs::write(dir.join("docs/stdlib.md"), "# Stdlib\n").expect("write stdlib doc");
    fs::write(dir.join("docs/backends.md"), "# Backends\n").expect("write backend doc");
    fs::write(dir.join("docs/topic(left.md"), "# Topic Left\n").expect("write topic doc");
    fs::write(dir.join("Progress/currentState.md"), "# State\n").expect("write state doc");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_serow"))
        ...
        .args(["docs", "--check", "--json"])
        .output()
        .expect("run serow docs --check with angle-bracketed parenthesized links");
    ...
    assert!(stdout.contains("\"markdown_links_ok\": false"), "{stdout}");
    assert!(
        stdout.contains("\"target\": \"docs/topic(left.md#missing-section\""),
        "{stdout}"
    );
    assert!(
        stdout.contains("\"resolved_path\": \"docs/topic(left.md#missing-section\""),
        "{stdout}"
    );
    assert!(
        !stdout.contains("\"target\": \"docs/topic(left.md#topic-left\""),
        "{stdout}"
    );
...
#[test]
fn docs_check_handles_escaped_angle_bracket_markdown_destinations() {
    let dir = unique_temp_dir("serow-docs-escaped-angle-destinations");
    fs::create_dir_all(dir.join("docs")).expect("create docs dir");
    fs::create_dir_all(dir.join("Progress")).expect("create progress dir");
    fs::write(
        dir.join("README.md"),
        concat!(
            "# Fixture\n\n",
            "Valid inline: [Topic](<docs/topic\\>right.md#topic-right> \"Topic\").\n",
            "Valid reference: [Topic Reference][topic-ref].\n",
            "Broken anchor: [Missing](<docs/topic\\>right.md#missing-section> \"Missing\").\n\n",
            "[topic-ref]: <docs/topic\\>right.md#topic-right> \"Topic\"\n"
        ),
    )
    .expect("write readme");
    fs::write(dir.join("AGENTS.md"), "# Agents\n").expect("write agents");
    ...
    fs::write(dir.join("docs/stdlib.md"), "# Stdlib\n").expect("write stdlib doc");
    fs::write(dir.join("docs/backends.md"), "# Backends\n").expect("write backend doc");
    fs::write(dir.join("docs/topic>right.md"), "# Topic Right\n").expect("write topic doc");
    fs::write(dir.join("Progress/currentState.md"), "# State\n").expect("write state doc");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_serow"))
        ...
        .args(["docs", "--check", "--json"])
        .output()
        .expect("run serow docs --check with escaped angle destinations");
    ...
    assert!(stdout.contains("\"markdown_links_ok\": false"), "{stdout}");
    assert!(
        stdout.contains("\"target\": \"docs/topic\\\\>right.md#missing-section\""),
        "{stdout}"
    );
    assert!(
        stdout.contains("\"resolved_path\": \"docs/topic>right.md#missing-section\""),
        "{stdout}"
    );
    assert!(
        !stdout.contains("\"target\": \"docs/topic\\\\>right.md#topic-right\""),
        "{stdout}"
    );
    assert!(
        !stdout.contains("\"resolved_path\": \"docs/topic\\\\\""),
        "{stdout}"
    );
...
#[test]
fn docs_check_handles_escaped_local_destination_separators() {
    let dir = unique_temp_dir("serow-docs-escaped-local-separators");
    fs::create_dir_all(dir.join("docs")).expect("create docs dir");
    fs::create_dir_all(dir.join("Progress")).expect("create progress dir");
    fs::write(
        dir.join("README.md"),
        concat!(
            "# Fixture\n\n",
            "Valid escaped hash: [Hash](docs/topic\\#hash.md#topic-hash).\n",
            "Broken escaped hash anchor: [Hash Missing](docs/topic\\#hash.md#missing-section).\n",
            "Valid escaped query: [Query][query-ref].\n",
            "Broken escaped query path: [Query Missing](docs/missing\\?query.md).\n\n",
            "[query-ref]: docs/topic\\?query.md#topic-query\n"
        ),
    )
    .expect("write readme");
    fs::write(dir.join("AGENTS.md"), "# Agents\n").expect("write agents");
    ...
    fs::write(dir.join("docs/stdlib.md"), "# Stdlib\n").expect("write stdlib doc");
    fs::write(dir.join("docs/backends.md"), "# Backends\n").expect("write backend doc");
    fs::write(dir.join("docs/topic#hash.md"), "# Topic Hash\n").expect("write hash doc");
    fs::write(dir.join("docs/topic?query.md"), "# Topic Query\n").expect("write query doc");
    fs::write(dir.join("Progress/currentState.md"), "# State\n").expect("write state doc");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_serow"))
        ...
        .args(["docs", "--check", "--json"])
        .output()
        .expect("run serow docs --check with escaped local separators");
    ...
    assert!(stdout.contains("\"markdown_links_ok\": false"), "{stdout}");
    assert!(
        stdout.contains("\"target\": \"docs/topic\\\\#hash.md#missing-section\""),
        "{stdout}"
    );
    assert!(
        stdout.contains("\"resolved_path\": \"docs/topic#hash.md#missing-section\""),
        "{stdout}"
    );
    assert!(
        stdout.contains("\"target\": \"docs/missing\\\\?query.md\""),
        "{stdout}"
    );
    assert!(
        stdout.contains("\"resolved_path\": \"docs/missing?query.md\""),
        "{stdout}"
    );
    assert!(
        !stdout.contains("\"target\": \"docs/topic\\\\#hash.md#topic-hash\""),
        "{stdout}"
    );
    assert!(
        !stdout.contains("\"target\": \"docs/topic\\\\?query.md#topic-query\""),
        "{stdout}"
    );
...
#[test]
fn docs_check_handles_root_relative_local_markdown_links() {
    let dir = unique_temp_dir("serow-docs-root-relative-links");
    fs::create_dir_all(dir.join("docs")).expect("create docs dir");
    fs::create_dir_all(dir.join("Progress")).expect("create progress dir");
    fs::write(
        dir.join("README.md"),
        concat!(
            "# Fixture\n\n",
            "Valid root-relative link: [Language](/docs/language.md#language).\n",
            "Broken root-relative link: [Missing](/docs/missing.md).\n"
        ),
    )
    .expect("write readme");
    fs::write(dir.join("AGENTS.md"), "# Agents\n").expect("write agents");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_serow"))
        ...
        .args(["docs", "--check", "--json"])
        .output()
        .expect("run serow docs --check with root-relative links");
...
#[test]
fn docs_check_reports_duplicate_reference_style_markdown_definitions() {
    let dir = unique_temp_dir("serow-docs-duplicate-reference-definitions");
    fs::create_dir_all(dir.join("docs")).expect("create docs dir");
    fs::create_dir_all(dir.join("Progress")).expect("create progress dir");
    fs::write(
        dir.join("README.md"),
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_serow"))
        ...
        .args(["docs", "--check", "--json"])
        .output()
        .expect("run serow docs --check with duplicate reference definitions");
    ...
    assert!(stdout.contains("\"target\": \"[language]\""), "{stdout}");
    assert!(
        stdout.contains("\"resolved_path\": \"duplicate reference definition `language`\""),
        "{stdout}"
    );
    assert!(
        !stdout.contains("missing reference definition `language`"),
        "{stdout}"
    );
    assert!(
        !stdout.contains("\"target\": \"docs/language.md#language\""),
...
#[test]
fn docs_check_handles_nested_markdown_link_labels() {
    let dir = unique_temp_dir("serow-docs-nested-link-labels");
    fs::create_dir_all(dir.join("docs")).expect("create docs dir");
    fs::create_dir_all(dir.join("Progress")).expect("create progress dir");
    fs::write(
        dir.join("README.md"),
        concat!(
            "# Fixture\n\n",
            "Valid nested inline: [See [Language]](docs/language.md#language).\n",
            "Broken nested inline: [See [Missing]](docs/language.md#missing-section).\n",
            "Valid nested reference: [See [CLI]][cli [reference]].\n",
            "Broken nested reference: [See [Missing]][missing [reference]].\n\n",
            "[cli [reference]]: docs/cli.md#cli\n",
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_serow"))
        ...
        .args(["docs", "--check", "--json"])
        .output()
        .expect("run serow docs --check with nested markdown labels");
    ...
    assert!(
        !stdout.contains("\"target\": \"docs/cli.md#cli\""),
        "{stdout}"
    );
    assert!(
        !stdout.contains("missing reference definition `cli [reference`"),
        "{stdout}"
    );
...
"#;
    ...
    assert_eq!(
        parse_cargo_manifest_version(&format!("{}[package]\nversion = \"1.2.3\"\n", '\u{00a0}')),
        None
    );
    assert_eq!(
        parse_cargo_manifest_version(&format!("[package]{}\nversion = \"1.2.3\"\n", '\u{00a0}')),
