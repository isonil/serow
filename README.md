...
- a first Rust backend source emitter for pure `Int`/`Bool`/`Text`/`Unit` functions and declared record types plus narrow terminal `io` intrinsics through `bin/serow compile rust`, with runtime assertions for Serow preconditions and postconditions, generated Rust tests for pure Serow examples and sampled properties, deterministic source-input/generated-source/type/binary/evidence metadata, and an optional runnable binary crate path for `pub fn main() -> Text | Int | Bool | Unit | <record>`
