...
- a first portable backend IR emitted by `bin/serow compile ir`, including checked implementation bodies, list literals, type/function source locations, `requires` preconditions, `ensures` postconditions, executable examples with source lines, and sampled properties with source lines
