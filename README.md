...
- a first Rust backend source emitter for pure `Int`/`Bool`/`Text` functions through `bin/serow compile rust`, with runtime assertions for Serow preconditions and postconditions, generated Rust tests for Serow examples and sampled properties, and deterministic generated-source fingerprint metadata
