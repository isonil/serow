...
`bin/serow query effects <symbol-or-name> [paths...] [--json]` reports a public function's declared effects, declared concrete capabilities, inferred concrete capabilities required by resolved direct callees, missing or unused direct-call capability deltas, the suggested canonical effect declaration, and the direct callees that contribute effect requirements. It includes compiler-owned intrinsics such as `print` and `read_line`, so terminal capability requirements can be audited without reading source text.
...
Effects are explicit on every public function. The bootstrap recognizes `effects pure` as a pure capability declaration and bracketed effect lists such as `effects [io]` or `effects [io, network]` as concrete capability declarations. `bin/serow check` rejects any direct call in an implementation, contract, example, or property when the caller does not declare every non-`pure` capability required by the resolved callee. This covers the pure case as the empty capability set: a `pure` caller cannot call an `[io]` callee, and an `[io]` caller cannot call a `[network]` callee unless it also declares `network`. The first compiler-owned terminal intrinsics are `print(text: Text) -> Unit` and `read_line() -> Text`; both require `io`, are queryable through the ledger, and do not require a source-level `use serow.intrinsic` declaration. The first compiler-owned pure list intrinsics are `len(list: List<T>) -> Int`, `contains(list: List<T>, value: T) -> Bool`, `push(list: List<T>, value: T) -> List<T>`, `remove_first(list: List<T>, value: T) -> List<T>`, plus temporary safe access intrinsics `get_text(list: List<Text>, index: Int) -> MaybeText`, `get_int(list: List<Int>, index: Int) -> MaybeInt`, `get_bool(list: List<Bool>, index: Int) -> MaybeBool`, and `get_float(list: List<Float>, index: Int) -> MaybeFloat`; they are queryable through the same ledger because source-level generics do not exist yet. Callers declare the bootstrap result records as `MaybeText = { found: Bool, value: Text }`, `MaybeInt = { found: Bool, value: Int }`, `MaybeBool = { found: Bool, value: Bool }`, and `MaybeFloat = { found: Bool, value: Float }` until generic payload enums can support `get(list, index) -> Option<T>`. Negative, out-of-range, and empty-list accesses return `found: false` with a deterministic placeholder value instead of panicking. The checker evaluates terminal intrinsics with a non-interactive model (`print` returns `unit` without writing, `read_line` returns empty `Text`) so examples and properties do not block local verification. The Rust backend is where terminal intrinsics perform real terminal I/O. The checker also warns with `UnusedEffectCapability` when a function has resolved non-self direct callees and declares concrete capabilities that none of those callees require; leaf effectful functions remain allowed.
...
`bin/serow patch add-type <path> <module> <type-declaration> [--json]` inserts one record or nullary enum type declaration into an existing module. The declaration argument may include the `type` prefix or just the right-hand declaration shape, such as `Player = { hp: Int, gold: Int }` or `Room = Hall | Cave`. The command rejects invalid module names, unknown module targets, duplicate type names, duplicate record field names, and duplicate enum variant names before rewriting the file through canonical formatting.

`bin/serow patch set-type <path> <module> <type-name> <type-declaration> [--json]` replaces one existing record type declaration's fields or one existing nullary enum type declaration's variants. The replacement declaration may include the `type` prefix, but its type name and record/enum kind must match the target type; use `patch rename-type` for renames, or `patch remove-type` plus `patch add-type` for deliberate record/enum kind changes. The command rejects invalid module or type names, unknown modules, missing type declarations, declaration/name mismatches, kind mismatches, duplicate field names, and duplicate enum variants before rewriting the file through canonical formatting.

`bin/serow patch remove-type <path> <module> <type-name> [--json]` removes one existing type declaration from a module in one source file. It validates module and type names, rejects unknown modules or missing type declarations, and rewrites the file through canonical formatting. Removing a referenced type can still surface checker diagnostics for the affected functions.
