...
`bin/serow compile ir [paths...] [--json]` is the first Phase 3 backend primitive. It parses the selected source paths, runs the normal checker, and only emits IR when there are no checker errors. Function rows include source path and line provenance. The checker and interpreter remain responsible for compile-time evidence: examples, contracts, preconditions, static expression types, effects, and sampled properties are validated before IR is produced.
