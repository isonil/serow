...
The current Rust backend supports pure public functions over `Int`, `Bool`, and `Text`. It maps `Int` to `i64`, `Bool` to `bool`, and `Text` to owned `String` values. It emits Rust for arithmetic, integer division and remainder, text concatenation, comparisons, boolean operators, unary operators, `if`, resolved calls to other generated functions, and runtime `assert!` guards for Serow `requires` preconditions. It also emits one Rust `#[test]` function for each checked Serow example, so generated crates can run those examples with `cargo test`. Canonical Serow symbols are lowered to deterministic Rust identifiers such as `@core.math.add.v1` -> `serow_core_math_add_v1`.
