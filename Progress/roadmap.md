...
- Add a Rust transpilation backend as the first production backend. _(Started: `bin/serow compile rust [paths...] [--out-dir <dir>] [--emit-bin] [--crate-name <name>] [--json]` emits deterministic Rust source or a minimal Rust crate layout with runtime contract assertions plus generated example and sampled-property tests for checked pure `Int`/`Bool`/`Text` functions from `serow.ir.v0`, supports configurable generated crate names, can write a runnable `src/main.rs` for `pub fn main() -> Text | Int | Bool`, records Serow backend metadata in generated Cargo manifests, and rejects effectful functions or invalid binary entrypoints with backend diagnostics.)_
