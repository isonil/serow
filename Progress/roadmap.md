...
- Strengthen property testing ergonomics:
  ...
  - treat shrinking for failing sampled properties as a stretch goal after replay is stable _(Started: `PropertyFailed` diagnostics now include deterministic `shrunk_sample_index`, `shrunk_sample_seed`, and `shrunk_bindings` data when a simpler failing sampled binding exists in the built-in sample set.)_
