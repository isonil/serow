...
- Add a Rust transpilation backend as the first production backend. _(Started: `bin/serow compile rust [paths...] [--json]` emits deterministic Rust source for checked pure `Int`/`Bool` functions from `serow.ir.v0`, and rejects unsupported `Text` or effectful functions with backend diagnostics.)_
