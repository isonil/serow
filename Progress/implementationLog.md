...
- Chose docs-check escaped angle-destination hardening because angle-bracket Markdown destinations can contain escaped punctuation, while local destination extraction still stopped at the first `>` and resolved escaped path punctuation literally.
- Added escape-aware angle destination closing plus local destination unescaping before percent decoding, covered inline and reference-style links to a local Markdown file containing `>` in its name, and bumped Serow to `1.0.41-rust-bootstrap` / crate `1.0.41`.

- Chose docs-check external URI hardening because the local Markdown link gate treated protocol-relative links and URI-scheme targets without `://`, such as `urn:`, as local filesystem paths.
- Added a scheme-aware external-target classifier, covered protocol-relative, `urn:`, and `mailto:` links while keeping broken local links reported, and bumped Serow to `1.0.40-rust-bootstrap` / crate `1.0.40`.
...
- Chose sampled-property memory hardening because checker, planner, shrinking, and Rust backend evidence generation all built full Cartesian sample products before iteration.
- Added a lazy Cartesian sample iterator that preserves existing sample order and one-based sample indexes, reused it across property checking, planning, shrinking, and generated Rust tests, and added focused iterator regressions for order, vacuous properties, and empty sample sets.
- Verified with `bin/serow query intent "iterate sampled property Cartesian bindings lazily without materializing every combination"`, `bin/serow query symbol "cartesian"`, `cargo fmt --check`, focused sampling/property/backend regressions, `cargo clippy --all-targets -- -D warnings`, `bin/serow check`, `bin/serow certify`, `bin/serow fmt --check --json`, `bin/serow certify --profile unattended`, `bin/serow docs --check --json`, `cargo test`, `python3 -m unittest discover -s tests`, `bin/serow release-check --json`, and `git diff --check`.

## 2026-06-02

- Chose Cargo manifest release-metadata hardening because `bin/serow release-check` depends on a dependency-free TOML reader that accepted bare `[package]` headers but missed valid quoted TOML table keys such as `["package"]`.
- Added quoted basic/literal TOML table-key parsing for the package-version reader, covered escaped quoted package keys, and bumped Serow to `1.0.33-rust-bootstrap` / crate `1.0.33`.
- Verified with `bin/serow query intent "release metadata reads Cargo package version from TOML manifest quoted table name"`, `bin/serow query symbol "release-check"`, `cargo fmt --check`, `cargo test cargo_manifest_version_parser_reads_package_version -- --nocapture`, `bin/serow check --json`, `bin/serow fmt --check --json`, `bin/serow docs --check --json`, `bin/serow certify --json`, `bin/serow certify --profile unattended --json`, `bin/serow release-check --json`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test`, `python3 -m unittest discover -s tests`, and `git diff --check`.

## 2026-06-04

- Chose sampled-property replay hardening because lazy property checking could handle empty sample sets without iteration, but direct one-based replay lookup still divided by zero when a later sampled binding had no values and could reject low valid samples when suffix Cartesian counts overflowed.
- Made `nth_cartesian_sample` reject any empty sample set up front and compute suffix counts with a caller-bounded saturating product, preserving existing sample ordering without requiring full Cartesian materialization.
- Added focused unit coverage for empty later sample sets and overflowing suffix products.
- Verified with `bin/serow query intent "general cleanup bugfix tech debt production readiness"`, `bin/serow query symbol "sample"`, `cargo fmt --check`, targeted `cargo test sampling::tests::nth_cartesian_sample -- --nocapture`, `bin/serow check --json`, `bin/serow certify --json`, `bin/serow certify --profile unattended --json`, `bin/serow docs --check --json`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test`, `python3 -m unittest discover -s tests`, `bin/serow release-check --json`, and `git diff --check`.
- Chose standard-library type-shape documentation drift prevention because `docs/stdlib.md` had regression coverage for every public function signature, but not for declared record types that are part of the same public source-level library surface.
...
- Verification is recorded in the final run for this change.
- Chose Rust backend property-sample diagnostic hardening because checker, replay, and plan paths already preserved unsupported deterministic sample reasons, while `compile rust` collapsed the same failure to a generic unsupported-type error.
- Reused the sampling unsupported-summary data in Rust backend generated-test emission so unsupported sampled property variables now report property index, variable name, unsupported types, unsupported reasons, and recursive record sample cycles when available.
- Added a compile-rust JSON regression using an unsupported sampled property binding to keep backend diagnostics aligned with checker/replay behavior.
- Verified with `bin/serow query intent "report unsupported deterministic property sample reasons in rust backend diagnostics"`, `bin/serow query symbol "sample_unsupported_summary"`, `cargo fmt --check`, targeted `cargo test sampled_property_with_unsupported_type_has_indexed_repair_action -- --nocapture`, `bin/serow check --json`, `bin/serow certify --json`, `cargo clippy --all-targets -- -D warnings`, `bin/serow fmt --check --json`, `bin/serow certify --profile unattended --json`, `cargo test`, `bin/serow docs --check --json`, `python3 -m unittest discover -s tests`, `bin/serow release-check --json`, and `git diff --check`.
- Chose project-manifest parser hardening because the dependency-free JSON reader rejected non-JSON text around the root object but could still skip arbitrary junk tokens before object keys and accept malformed `serow.project` metadata.
