...
- Phase 3 backend foundation:
  ...
  - includes public symbol identity, signature, effects, parameters, return type, lowered `requires` preconditions, lowered `ensures` postconditions, lowered executable examples, lowered sampled properties, expression tree, and canonical resolved call targets
...
- `serow compile rust` emits deterministic Rust source and can write a minimal Rust crate layout for pure checked `Int`/`Bool`/`Text` functions, including runtime `requires`/`ensures` assertions and generated Rust tests for checked examples and deterministic sampled-property bindings, but effectful functions, ownership-friendly state transforms, WASM, TypeScript, Python, and richer backend package metadata do not exist yet.
