...
- Phase 3 Rust backend:
  ...
  - records deterministic `package.metadata.serow` manifest rows for the backend id, IR version, generated source fingerprint, generated type/function/test counts, source-location-aware type and function symbol-to-Rust-name mappings, binary entrypoint symbol/Rust-name/source-location metadata, and source-location-aware example/property evidence-to-test mappings in generated crates
  ...
  - avoids whole-record clones for direct field reads from local record variables and lowers same-variable record state updates such as `set state = state with { hp: state.hp - 1 }` to in-place Rust field assignments after evaluating update values
