...
- Latest closure: Serow is at `1.0.2-rust-bootstrap` after a public v1 patch release that makes malformed `release-check` option usage return structured `UsageError` diagnostics instead of flowing into source discovery. `bin/serow release-check [paths...] [--json]` runs the Serow-owned public v1 release gates in one command: documentation reference validation, canonical formatting, standard certification, and unattended certification.
