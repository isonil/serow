...
- Latest closure: Serow is at `1.0.15-rust-bootstrap` after a public v1 patch release that makes `bin/serow docs` advertise the project overview alongside the rest of the stable local references.
...
- Documentation discovery command:
  ...
  - lists the stable local project overview, language, CLI, standard library, backend, agent-instruction, and progress references in text or machine-readable form, and can fail CI-style when an advertised reference is missing or a public local Markdown link points at a missing file or missing heading anchor
