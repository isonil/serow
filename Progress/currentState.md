...
- Latest cleanup: docs-check escaped angle-destination handling now finds escaped `>` closers correctly and unescapes local destination punctuation before percent decoding, so links to Markdown files whose names contain escaped punctuation are validated against the actual path.
...
- Checker for:
  ...
  - sampled `forall` properties over deterministic `Int`, `Float`, `Bool`, `Text`, singleton `Unit`, bounded homogeneous `List<T>`, bounded declared-record, and declared-enum sample sets, with unsupported-sample reasons surfaced through check, replay, and plan output, including exact unknown type names
...
- Compiler-owned pure list intrinsics:
  ...
  - temporary safe access for current bootstrap types with `get_text(list: List<Text>, index: Int) -> MaybeText`, `get_int(list: List<Int>, index: Int) -> MaybeInt`, `get_bool(list: List<Bool>, index: Int) -> MaybeBool`, and `get_float(list: List<Float>, index: Int) -> MaybeFloat`
  - callers declare `MaybeText = { found: Bool, value: Text }`, `MaybeInt = { found: Bool, value: Int }`, `MaybeBool = { found: Bool, value: Bool }`, and `MaybeFloat = { found: Bool, value: Float }`, or use the concrete stdlib wrappers, until generic payload enums can support `get(list, index) -> Option<T>`
...
- Phase 3 Rust backend:
  ...
  - lowers checked terminal `io` intrinsics to Rust `println!` and stdin line reading that returns empty text on read failure
...
- `patch set-type` replaces one existing record type declaration's fields or one existing nullary enum type declaration's variants through the structured patch interface, rejects declaration/name and record/enum kind mismatches so renames and kind changes stay explicit, and preserves canonical formatting.
