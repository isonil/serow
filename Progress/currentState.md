...
- Phase 3 backend foundation:
  ...
  - includes public symbol identity, signature, effects, parameters, return type, lowered `requires` preconditions, lowered executable examples, expression tree, and canonical resolved call targets
- Phase 3 Rust backend:
  ...
  - emits generated Rust `#[test]` functions for checked Serow examples, and reports symbol/example-to-test mappings in JSON mode
...
- `serow compile rust` emits deterministic Rust source and can write a minimal Rust crate layout for pure checked `Int`/`Bool`/`Text` functions, including generated Rust tests for checked examples, but effectful functions, ownership-friendly state transforms, WASM, TypeScript, Python, and richer backend package metadata do not exist yet.
