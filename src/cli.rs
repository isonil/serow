...
fn run_release_check(args: &[String]) -> i32 {
    let PathArgs { paths, json_output } = match parse_paths_and_json(args, "release-check") {
        Ok(parsed) => parsed,
...
fn write_rust_crate_artifact(artifact: &RustCrateArtifact) -> Result<Vec<String>, Box<Diagnostic>> {
    ...
    for file in &artifact.files {
        ensure_generated_backend_file_can_be_written(file, &artifact.out_dir)?;
    }
    for path in &artifact.absent_generated_files {
        ensure_stale_generated_backend_file_can_be_removed(path, &artifact.out_dir)?;
    }
    write_backend_file(
        &src_dir,
...
fn ensure_generated_backend_file_can_be_written(
    file: &RustCrateArtifactFile,
    out_dir: &str,
) -> Result<(), Box<Diagnostic>> {
    match fs::read_to_string(&file.path) {
        Ok(source) if is_generated_rust_crate_artifact(&file.path, &source) => {}
        Ok(_) => {
            return Err(Box::new(
                Diagnostic::error(
                    "RustBackendUnexpectedArtifact",
                    format!(
                        "Refusing to overwrite hand-authored Rust backend artifact `{}`.",
                        file.path.display()
                    ),
                    Some(out_dir.to_string()),
                )
                .with_data("path", file.path.display().to_string())
                .with_data("expected", "generated")
                .with_repair("Choose an empty `--out-dir` or remove the hand-authored file."),
            ));
        }
        ...
        Err(error) => {
            return Err(Box::new(
                Diagnostic::error(
                    ...
                    format!(
                        "Could not read Rust backend artifact `{}` before writing: {error}",
                        file.path.display()
                    ),
                    Some(out_dir.to_string()),
                )
...
fn ensure_stale_generated_backend_file_can_be_removed(
    path: &Path,
    out_dir: &str,
) -> Result<(), Box<Diagnostic>> {
    match fs::read_to_string(path) {
        Ok(source) if is_generated_rust_binary_entrypoint(&source) => Ok(()),
        Ok(_) => Err(Box::new(
            Diagnostic::error(
                "RustBackendUnexpectedArtifact",
                format!(
...
fn is_generated_rust_crate_artifact(path: &Path, source: &str) -> bool {
    match path.file_name().and_then(|name| name.to_str()) {
        Some("Cargo.toml") => {
            source.starts_with("# Generated by `serow compile rust --out-dir`.\n")
        }
        Some("README.md") => source.lines().nth(2).is_some_and(|line| {
            line == "Generated by `serow compile rust --out-dir` from checked Serow sources."
        }),
        Some("serow-metadata.json") => is_generated_rust_metadata_artifact(source),
        Some("lib.rs") => {
            source.starts_with("// Generated by `serow compile rust` from checked serow.ir.v0.\n")
        }
        Some("main.rs") => is_generated_rust_binary_entrypoint(source),
        _ => false,
    }
}

fn is_generated_rust_metadata_artifact(source: &str) -> bool {
    source.starts_with("{\n  \"backend\": \"serow.rust.v0\",\n")
        && source.contains("\n  \"crate_name\": ")
        && source.contains("\n  \"generated_counts\": ")
