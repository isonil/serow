...
fn run_compile_rust(args: &[String]) -> i32 {
    ...
    if let (Some(out_dir), Some(rust)) = (&parsed.out_dir, &summary.rust) {
        match write_rust_crate(out_dir, &parsed.crate_name, rust) {
...
fn write_rust_crate(
    ...
    let cargo_source = render_generated_cargo_toml(crate_name);
...
fn render_generated_cargo_toml(crate_name: &str) -> String {
    format!(
        concat!(
...
const FULL_AGENT_COMMANDS: &[AgentCommand] = &[
    ...
    (
        ...
        "Emit deterministic Rust source or a generated Rust crate with configurable package name plus example and sampled-property tests for the supported checked IR subset.",
    ),
    (
        "fmt",
        "serow fmt [paths...] [--check] [--json]",
...
fn print_usage() {
    ...
    eprintln!("  serow compile rust [paths...] [--out-dir <dir>] [--crate-name <name>] [--json]");
