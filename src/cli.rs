...
fn validate_binary_entrypoint_shape(
    ...
    let Some(function) = entrypoints.first() else {
        return Err(vec![
            ...
            .with_data("expected", "pub fn main() -> Text | Int | Bool"),
        ]);
    };
    ...
    if !matches!(function.return_type.as_str(), "Text" | "Int" | "Bool") {
        diagnostics.push(
            Diagnostic::error(
                "RustBinaryUnsupportedEntrypointReturn",
                format!(
                    "Rust binary entrypoint `{}` returns unsupported type `{}`; expected Text, Int, or Bool.",
                    function.symbol(),
            ...
            .with_data("supported_return_types", "Text, Int, Bool"),
        );
    }
...
fn print_agent_bootstrap() {
    ...
    println!("  Rust binary emission requires pub fn main() -> Text | Int | Bool");
