...
fn run_compile_rust(args: &[String]) -> i32 {
    ...
    if let (Some(out_dir), Some(rust)) = (&parsed.out_dir, &summary.rust) {
        match write_rust_crate(out_dir, rust) {
            Ok(files) => written_files = files,
            Err(diagnostic) => summary.diagnostics.push(*diagnostic),
        }
    }
    if json_output {
        println!("{}", rust_summary_json(&summary, &written_files));
...
fn rust_summary_json(summary: &RustBackendSummary, written_files: &[String]) -> String {
    format!(
        concat!(
            "{{\n",
            "  \"diagnostics\": {},\n",
...
fn print_usage() {
    ...
    eprintln!("  serow compile rust [paths...] [--out-dir <dir>] [--json]");
