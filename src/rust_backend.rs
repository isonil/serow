...
fn generate_rust_program(ir: &IrProgram) -> Result<GeneratedRustProgram, Vec<Diagnostic>> {
    ...
    if generated_source_uses_float_helper(&rendered_functions, &rendered_tests) {
        source.push_str(generated_float_helpers_source());
        source.push_str("\n\n");
    }
...
fn generated_float_helpers_source() -> &'static str {
    concat!(
        "fn serow_finite_float(value: f64) -> f64 {\n",
        "    assert!(value.is_finite(), \"Serow Float result must be finite\");\n",
        "    value\n",
        "}"
    )
}

fn generated_source_uses_float_helper(functions: &[String], tests: &[String]) -> bool {
    functions
        .iter()
        .chain(tests)
        .any(|source| source.contains("serow_finite_float("))
}
...
fn render_expr(
    ...
    match expr {
        ...
        IrExpr::Binary { op, left, right } => {
            ...
            let code = format!("({} {operator} {})", left.code, right.code);
            let code = if type_name == "Float" {
                format!("serow_finite_float({code})")
            } else {
                code
            };
            Ok(rendered(code, type_name))
        }
        IrExpr::If {
            condition,
            then_expr,
