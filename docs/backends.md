...
`compile rust` runs the same checked IR path first, then emits deterministic Rust source on stdout. With `--out-dir <dir>` or `--out-dir=<dir>`, it writes a dependency-free Rust crate layout containing `Cargo.toml`, `README.md`, `serow-metadata.json`, and `src/lib.rs`; library-only generation removes a stale Serow-generated `src/main.rs` from an earlier `--emit-bin` run so Cargo does not keep an unintended binary target. `--check-out-dir` compares generated artifacts against an existing output directory without writing and reports `RustBackendArtifactDrift`, `RustBackendMissingArtifact`, or `RustBackendStaleGeneratedArtifact` diagnostics when expected generated files drift, are missing, or a Serow-generated optional artifact is no longer expected. Non-Serow unexpected files are reported as `RustBackendUnexpectedArtifact` and are never removed automatically. `--crate-name <name>` or `--crate-name=<name>` customizes the generated Cargo package name and defaults to `serow_generated`; custom names must start with a lowercase ASCII letter, use only lowercase ASCII letters, digits, `_`, or `-`, and avoid adjacent separators so Cargo's normalized library crate name stays warning-clean.
