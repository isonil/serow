...
Lists lower to Rust `Vec<T>` and support literals, equality, `len(list)`, `contains(list, value)`, `push(list, value)`, `remove_first(list, value)`, and temporary safe access intrinsics `get_text(list: List<Text>, index: Int) -> MaybeText`, `get_int(list: List<Int>, index: Int) -> MaybeInt`, and `get_bool(list: List<Bool>, index: Int) -> MaybeBool`; callers declare `MaybeText = { found: Bool, value: Text }`, `MaybeInt = { found: Bool, value: Int }`, or `MaybeBool = { found: Bool, value: Bool }`, or use the concrete stdlib wrappers, until generic payload enums make `get(list, index) -> Option<T>` possible. Negative, out-of-range, and empty-list indexes return `found: false` with a stable placeholder value (`""`, `0`, or `false`) instead of panicking. Iteration, generic indexing, map/filter/fold, slicing, and list forall sample generation are not in this slice.
