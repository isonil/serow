...
`docs check` is accepted as an alias for `docs --check`. Check mode exits non-zero if any advertised local documentation path is missing, if an inline or reference-style local Markdown link in the public docs points at a missing file or heading anchor, if a full/collapsed reference-style link usage has no definition, or if a reference-style label is defined more than once. Inline and reference-style links may use normal Markdown titles, and local link paths may be relative to the source file, root-relative to the checked project, or percent-encoded with path characters such as `%20`. External URI links, including protocol-relative links, are ignored. Link-like syntax inside fenced code blocks, inline backtick code spans, or escaped Markdown text is ignored, and heading-like lines inside fenced code blocks do not satisfy anchor checks. Anchor checks recognize ATX `# Heading` and setext underlined headings. The advertised references include the project overview, language, CLI, standard library, backend, agent-instruction, and progress documents. JSON output includes an `exists` field per reference plus top-level `references_ok`, `missing`, `markdown_links_ok`, and `broken_links` fields.
...
`release-check` validates release metadata consistency between `serow.project` and `Cargo.toml`, validates advertised docs plus local documentation files, reference definitions, and heading anchors, checks canonical formatting, runs standard certification, and runs unattended certification over the selected source paths. It does not wrap repository-level Rust or Python test commands.
